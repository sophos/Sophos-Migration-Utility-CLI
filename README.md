...
- -A - Optional flag to enable physical Interface, VLAN, and additional-address Alias export. Default: off. Exports static IPv4 interfaces/VLANs as static, DHCP/dynamic interfaces/VLANs as DHCP without converting current leases to static IPs, maps UTM `ethN` hardware to SFOS `PortN+1`, emits SFOS-required physical-interface defaults for IPv6 assignment, MTU/MSS, and link settings, defaults zones to `LAN`, carries over the source hardware's MAC address when available (falling back to `Default`), always imports interfaces and VLANs administratively disabled for the operator to review and enable, and warns for PPPoE, IPv6, disabled, or incomplete address data that is not migrated.
...
17. When using the -A option to include interfaces and VLANs, the operator-facing UTM interface/VLAN name is preserved in the SFOS `<Name>` field (the `PortN`/`PortN.V` binding is kept in `<Hardware>`, which is what all references resolve against). It is still necessary to verify the zone and other details of the objects before importing into SFOS. SFOS enforces a 60-character UNIQUE constraint on interface names, so any over-length or duplicate UTM names are exported verbatim and flagged with a warning for you to resolve. Processing via Sophos Firewall Config Studio is recommended when using this option.
18. **Known limitation:** SFOS rejects interface/VLAN names containing `'`, `"`, `<`, `>`, `#`, `%`, `/`, `\`, or `,`. Because `-A` preserves the UTM name verbatim, a name containing any of these characters fails SFOS's import outright and the interface/VLAN is left unbound rather than imported.
